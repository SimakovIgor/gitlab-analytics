...
**Страница DORA** (`GET /dora`): отображает 4 DORA-метрики. `DoraService` — центральный сервис, методы `buildLeadTimeData()`, `buildDeployFrequencyData()`, `buildChangeFailureRateData()`,
`buildMttrData()`. Каждый возвращает `Map<String, Object>` с данными для карточки, графика и спарклайна. `DoraController` передаёт данные в Thymeleaf-шаблон `dora.html`. Фильтры: период (`PeriodType`)
+ проекты (`projectIds`).
