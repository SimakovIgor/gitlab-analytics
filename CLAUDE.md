...
evaluator-ам и содержит: `users`, `current`/`previous` `Map<Long, UserMetrics>`, `openMrs`, `gitlabUserIdToTrackedUserId`. Результаты сортируются по `severity` по убыванию.

**`InsightRule` enum** (`insights/model/InsightRule.java`) — единственный источник истины для всех 10 правил (аналог `Metric` enum). Каждое правило имеет `code()`, `defaultKind()` (
...
`merge-time-spike-ratio`, `review-gini`, `large-mr-lines`, `delivery-drop-ratio`, `min-comments-per-mr`, `max-rework-ratio`, `min-mrs-for-no-review-check`.
