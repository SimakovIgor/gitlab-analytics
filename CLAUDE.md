...
`leadTimeMedianDays`/`prevLeadTimeMedianDays`, `deploysPerDay`/`prevDeploysPerDay`. Результаты сортируются по `severity` по убыванию.
...
`min-deploys-per-day`, `lead-time-regression-ratio`.
...
**Страница DORA** (`GET /dora`): отображает 4 DORA-метрики. `DoraService` — центральный сервис, методы `buildLeadTimeData()`, `buildDeployFrequencyData()`, `buildChangeFailureRateData()`. Каждый
возвращает `Map<String, Object>` с данными для карточки, графика и спарклайна. `DoraController` передаёт данные в Thymeleaf-шаблон `dora.html`. Фильтры: период (`PeriodType`) + проекты
(`projectIds`).

**DoraMetric enum** (`dora/model/DoraMetric.java`) — источник истины для 4 DORA-метрик. Каждая имеет `label()`, `description()`, `status()` (AVAILABLE/COMING_SOON). При добавлении новой метрики:
обновить enum, добавить `build*Data()` метод в `DoraService`, подключить в `DoraController`, добавить карточку/график в `dora.html`.
...
**Jira-интеграция** (Change Failure Rate): инциденты синхронизируются из Jira Server/DC через REST API v2 (`/rest/api/2/search`). Бины `JiraApiClient` и `JiraIncidentSyncService` создаются
условно — только если `app.jira.base-url` непуст (`@ConditionalOnExpression`). Маппинг инцидентов к проектам: поле `components` в Jira сопоставляется с `TrackedProject.name` (case-insensitive).
Один инцидент может маппиться на несколько проектов (денормализовано: одна строка на `jira_key + tracked_project_id`). Эндпоинт `POST /dora/sync/incidents` запускает синк из UI; если Jira не
сконфигурирован, возвращает `"disabled"`.
