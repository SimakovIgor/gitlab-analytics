...
- **MR-метрики** (mr_merged_count, review_time и др.) — атрибуция по GitLab user ID. При добавлении сотрудника система автоматически ищет в GitLab API пользователя по username-части email (например, `a.upatov` из `a.upatov@uzum.com`) и сохраняет его ID.
