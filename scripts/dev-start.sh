...
[[ -n "${SPRING_MAIL_USERNAME:-}" ]] && APP_ENV+=(SPRING_MAIL_USERNAME="$SPRING_MAIL_USERNAME")
[[ -n "${SPRING_MAIL_PASSWORD:-}" ]] && APP_ENV+=(SPRING_MAIL_PASSWORD="$SPRING_MAIL_PASSWORD")
if [[ "$WITH_MONITORING" == true ]]; then
  APP_ENV+=(SPRING_PROFILES_ACTIVE="dev,monitoring")
fi
